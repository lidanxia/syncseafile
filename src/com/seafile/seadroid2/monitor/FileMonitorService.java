 ...
 * 主要是在收到文件的相关信息时，进行相应的处理，比如说文件上传成功后，下载成功等
 */
public class FileMonitorService extends Service 
{
    ...
    //其它程序调用startService()后，本service调用该函数启动服务
    {
   ...
   //其它程序调用bindService()后，该service先运行onCreate(),再运行onBind()启动服务
    @Override
...
/**
 * transferReceiver广播接收器，当接收到文件下载或者上传成功的信息时，采取相应的操作
 * 下载成功时，将其加入到监控map中，上传成功时，就将其从自动更新表中删除
 */
    private BroadcastReceiver transferReceiver = new BroadcastReceiver() 
    {
        ...
        public void onReceive(Context context, Intent intent) 
        {
            ...
            if (type.equals(TransferService.BROADCAST_FILE_DOWNLOAD_SUCCESS))
            {
                ...
                System.out.println("FileMonitorServices--下载文件后，对本地文件进行监听");
            ...
            {
            	System.out.println("FileMonitorServices--上传文件后，对本地文件进行监听");
                int taskID = intent.getIntExtra("taskID", 0);
                ...
                if(info.isUpdate==false)
                {
                	 monitor.onFileuploaded(info.account, info.repoID, info.repoName,
                			 info.parentDir, info.localFilePath);

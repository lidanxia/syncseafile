 ...
 * 定义了TransferListener
 * 调用了 dataManager的方法实现了上传，下载文件
 * Currently use an AsyncTask--类似于线程 for an file.
 */
public class TransferManager {

    @SuppressWarnings("unused")
    private static final String DEBUG_TAG = "TransferManager";
    public static boolean isreqdown=false;
    ...
    private ArrayList<UploadTask> uploadTasks;  //记载待上传的文件信息，上传之后就会删除
    public static ArrayList<DownloadTask> downloadTasks;  //可能有错
                              ...
                              String dir, String filePath, boolean isUpdate) 
    {
       ...
       // System.out.println("####上传完成后，上传列表的大小："+uploadTasks.size());
        System.out.println("TransferManager:addUploadTask,上传线程开始执行");
                               ...
                               String path) 
    {
       ...
       // System.out.println("####下载完成后，下载列表的大小："+downloadTasks.size());
        DownloadTask task = new DownloadTask(account, repoName, repoID, path);
        System.out.println("下载文件时，  DownloadTask的参数path： "+path);
        task.execute();
        return task.getTaskID();
    }
/**
 ...
 */
    ...
    {
    	// System.out.println("downloadTasks.size():    "+downloadTasks.size());
    ...
    {
    	 DownloadTask task = getDownloadTaskByID(taskID);
      //   System.out.println("getDownloadTaskInfo---task:    "+task);
        if (task != null) {
            return task.getTaskInfo();
        }
        return null;
   ...
   /**
    * 上传线程，UploadTask的对象执行execute（）时，线程运行
    * 调用dataManager.uploadFile（）执行上传任务
    * @author PC
    *
    */
    private class UploadTask extends AsyncTask<String, Long, Void> 
    {
                          ...
                          String dir, String filePath, boolean isUpdate) 
        {
           ...
           // Log.d(DEBUG_TAG, "stored object is " + myPath + myObjectID);
            ...
            System.out.println("%%%添加上传任务 uploadTasks.size() "+  uploadTasks.size());
            err = null;
        }
        ...
        protected Void doInBackground(String... params)
        {
            try {
                ...
                if (isUpdate) 
                {
                    ...
                    //上传文件
                    System.out.println("TransferManager:更新上传的文件:UploadTask");
                } 
                else 
                {
                    dataManager.uploadFile(myRepoName, myRepoID, myDir, myPath, monitor);
                    System.out.println("TransferManager:上传最新的文件:UploadTask");
                }
            } 
            catch (SeafException e)
            {
                ...
                err = e;
                System.out.println("TransferManager:上传文件出现错");
            }
        ...
        protected void onPostExecute(Void v) 
        {
            ...
            System.out.println("是否出错或是完成   ："+myState);
            if (listener != null)
            {
                if (err == null) 
                {
...
/**
 * 下载线程。DownloadTask的对象执行execute（）时，线程运行
 * 调用dataManager.getFile（）下载文件
 * @author PC
 *
 */
    private class DownloadTask extends AsyncTask<String, Long, File>
    {
        ...
        public DownloadTask(Account account, String repoName, String repoID, String path)
        {
            ...
            System.out.println("%%%添加下载任务 downloadTasks.size() "+  downloadTasks.size());
            err = null;
            //只要添加了下载任务就说明要下载，没等下载完就将onfilechange哪里设为true
            isreqdown=true;

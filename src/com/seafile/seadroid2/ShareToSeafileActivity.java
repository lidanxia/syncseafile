...
/**
 * 根据跳转到该Activity的intent参数的uri,获取到文件的本地路径，选定文件，上传到服务器
 * @author PC
 *
 */
 ...
 */
    ...
    public void onListItemClick(final View v, final int position, final long id)
    {
        ...
        {
            repo = getReposAdapter().getItem(position); //当前选定的仓库
        } 
...
/**
 * 设定account为当前账号，并传到mDataManager中
 * @param account
 */
    ...
    private class LoadDirTask extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            try {
            	 System.out.println("ShareToActivity-LoadDirTask");
